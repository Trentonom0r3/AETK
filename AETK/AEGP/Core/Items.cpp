...
std::shared_ptr<AE::Item> AE::Item::activeItem()
{
    ...
    // Asynchronous path
    auto futureResult =
        TaskScheduler::GetInstance().ScheduleTask<ItemPtr>([]() -> ItemPtr { return ItemSuite9().GetActiveItem(); },
                                                           false); // CALLIDLE is True
    auto item = futureResult.get();
...
bool AE::Item::Missing()
{
    auto flags = m_itemSuite.GetItemFlags(m_item);
    AEGP_ItemFlags flag = static_cast<AEGP_ItemFlags>(flags);
    return flag & AEGP_ItemFlag_MISSING;
}

bool AE::Item::HasProxy()
{
    auto flags = m_itemSuite.GetItemFlags(m_item);
    AEGP_ItemFlags flag = static_cast<AEGP_ItemFlags>(flags);
    return flag & AEGP_ItemFlag_HAS_PROXY;
}

bool AE::Item::UsingProxy()
{
    auto flags = m_itemSuite.GetItemFlags(m_item);
    AEGP_ItemFlags flag = static_cast<AEGP_ItemFlags>(flags);
    return flag & AEGP_ItemFlag_USING_PROXY;
}

bool AE::Item::MissingProxy()
{
    auto flags = m_itemSuite.GetItemFlags(m_item);
    AEGP_ItemFlags flag = static_cast<AEGP_ItemFlags>(flags);
    return flag & AEGP_ItemFlag_MISSING_PROXY;
}

bool AE::Item::HasVideo()
{
    auto flags = m_itemSuite.GetItemFlags(m_item);
    AEGP_ItemFlags flag = static_cast<AEGP_ItemFlags>(flags);
    return flag & AEGP_ItemFlag_HAS_VIDEO;
}

bool AE::Item::HasAudio()
{
    auto flags = m_itemSuite.GetItemFlags(m_item);
    AEGP_ItemFlags flag = static_cast<AEGP_ItemFlags>(flags);
    return flag & AEGP_ItemFlag_HAS_AUDIO;
}
...
bool AE::Item::Still()
{
    auto flags = m_itemSuite.GetItemFlags(m_item);
    AEGP_ItemFlags flag = static_cast<AEGP_ItemFlags>(flags);
    return flag & AEGP_ItemFlag_STILL;
}

bool AE::Item::HasActiveAudio()
{
    auto flags = m_itemSuite.GetItemFlags(m_item);
    AEGP_ItemFlags flag = static_cast<AEGP_ItemFlags>(flags);
    return flag & AEGP_ItemFlag_HAS_ACTIVE_AUDIO;
}
...
AE::FolderItem AE::FolderItem::create(const std::string &name, std::shared_ptr<FolderItem> parentFolder)
{
...
AE::CompItem AE::CompItem::create(const std::string &name, int width, int height, double pixelAspect, double duration,
                                  double frameRate, std::shared_ptr<FolderItem> item)
{
    auto comp = CompSuite11().CreateComp(item->getItem(), name, width, height, {static_cast<int>(pixelAspect), 1},
...
AE::FootageItem AE::FootageItem::createPlaceholder(const std::string &path, int width, int height, double duration,
                                                   AE_Platform plat)
{
    auto footage = FootageSuite5().newPlaceholderFootage(path, width, height, SecondsToTime(duration));
    auto footageItem = FootageSuite5().addFootageToProject(footage, ItemPtr());
    return FootageItem(footageItem);
}

AE::FootageItem AE::FootageItem::createSolid(const std::string &name, int width, int height, ColorVal color)
{
