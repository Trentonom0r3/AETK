...
class Command
{
  public:
    ...
    Command(std::string name, MenuID menuID, int after_item = AEGP_MENU_INSERT_SORTED)
        : m_name(name), m_command(CommandSuite().getUniqueCommand())
    {
    ...
    {
        CommandSuite().insertMenuCommand(m_command, m_name.c_str(), menuID, after_item);
    }
    inline void setCommandName(std::string name) { CommandSuite().setMenuCommandName(m_command, name.c_str()); }
    inline void enableCommand(bool enable)
    {
        if (enable)
        {
            CommandSuite().enableCommand(m_command);
        }
        else
        {
            CommandSuite().disableCommand(m_command);
        }
    }

    inline void checkCommand(bool check) { CommandSuite().checkMarkMenuCommand(m_command, check); }
